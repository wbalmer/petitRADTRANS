...
def make_table_ck_species_reference_documentation(path_to_ck_opacities=None):
    ...
    for species in species_list:
        ...
        name, _, charge, cloud_info, source, _ = split_species_all_info(file_name)
        ...
        with h5py.File(species, 'r') as f:
            ...
            contributor = '--'
            try:
                cID = f['DOI'].attrs['contributor']
                if cID is not None:
                    if 'karan' in cID:
                        contributor = '`K. Molaverdikhani <mailto:karan.molaverdikhani@colorado.edu>`_'
                    elif 'exomol' in cID:
                        contributor = '`Chubb et al. (2021) <https://ui.adsabs.harvard.edu/abs/2021A%26A...646A..21C/abstract>`_, `ExoMolOP <https://www.exomol.com/data/data-types/opacity/>`_'
            except KeyError:
                pass

        call_name_short = call_name.split('__')[0]

        # TODO add charge information here
        return_string += (
            f"\n"
            f"    * - {call_name_short}\n"
            f"      - {call_name}\n"
