...
module fortran_radtrans_core
    ...
    contains
        ...
        subroutine compute_feautrier_radiative_transfer(frequencies_bin_edges, temperatures, weights_gauss, &
            ...
            integer                       :: n_iterations_min(n_frequencies_bin_edges-1)
            ! integer                       :: adaptive_n_iterations_min(n_frequencies_bin_edges-1)
            ...
            double precision                :: I_plus_surface(n_angles, n_g, n_frequencies_bin_edges-1), &
                                               mu_double(n_angles), geometry_coefficient
            double precision :: dtau_1_level(n_layers - 1), dtau_2_level(n_layers - 2)
            ...
            if (trim(adjustl(emission_geometry)) == 'dayside_ave') then
                geometry_coefficient = 0.5d0
            else if (trim(adjustl(emission_geometry)) == 'planetary_ave') then
                geometry_coefficient = 0.25d0
            else if (trim(adjustl(emission_geometry)) == 'non-isotropic') then
                geometry_coefficient = -1d0
            else
                write(*,*) 'Invalid geometry'
            end if

            if (geometry_coefficient > 0) then
                do i = 1, n_frequencies_bin_edges-1
                    do i_mu = 1, n_angles
                        I_star_calc(:, :, i_mu, i) = geometry_coefficient * abs(stellar_intensity(i)) &
                ...
                do i = 1, n_frequencies_bin_edges-1
                    J_star_ini(:, :, i) = abs(&
                        stellar_intensity(i) / 4d0 * exp(-optical_depths(:,i,:)/star_irradiation_cos_angle)&
                    )
                end do
            end if

            ! Prevent issues with negative or 0 photon destruction probabilities
            do i = 1, n_frequencies_bin_edges-1
                do l = 1, n_g
                    do k = 1, n_layers
                ...
                if (adaptive_feautrier_iterations) then
                    ! Calculate the minimum number of iterations for the current frequency
                    ! based on the photon destruction probabilities
                    n_iterations_min(i) = &
                        1 + int(maxval(1d0/photon_destruction_probabilities_(:,:,i)))

                    if (n_iterations_min(i) < 4) then
                        ! Run the Ng acceleration at least once
                        n_iterations_min(i) = 4
                    else if (n_iterations_min(i) > 9) then
                        ! In case the number of iterations exceeds the maximum
                        n_iterations_min(i) = 9
                    end if
                else
                    ! Use the default
                    n_iterations_min(i) = 9
                ...
                do l = 1, n_g
                    if  (trim(adjustl(emission_geometry)) /= 'non-isotropic') then
                        ...
                        I_surface_reflection = reflectances(i) &
                            * J_star_ini(l, n_layers, i) * 4d0 * star_irradiation_cos_angle
                    end if
            ...
            do i = 1, n_frequencies_bin_edges - 1
                ...
                do l = 1, n_g
                    ...
                    ! Iterate flux calculation until it has converged
                    do i_iter_scat = 1, iter_scat
                        do j = 1, n_angles
                            ...
                            ! Sum to get I_J
                            r(n_layers) = 0.5d0 * (I_plus_surface(j, l, i) + I_minus)

                            ! Solve the Feautrier problem
                            call solve_tridiagonal_system(&
                                a(:, j), b(:, j), c(:, j), r, n_layers, I_J(:,j) &
                            )

                            I_H(1, j) = -I_J(1, j)

                            do k = 2, n_layers - 1
                                deriv1 = f2_struct(k, j) * (I_J(k + 1, j) - I_J(k, j))
                                deriv2 = f3_struct(k, j) * (I_J(k, j) - I_J(k - 1, j))
                        ...
                        if ((conv_val < convergence_threshold) .and. (i_iter_scat > n_iterations_min(i))) then
                            exit
                        end if
                        ...
                        if (mod(i_iter_scat, 4) == 0) then
                            source_planet_scat_n = source(:, l, i)

                            call m_approximate_ng_source(source_planet_scat_n,source_planet_scat_n1, &
                                source_planet_scat_n2,source_planet_scat_n3,source(:, l, i), &
                                1,1,n_layers)
                        end if
            ...
            if (return_contribution) then
                do i_mu = 1, n_angles
                    ...
                    do i_freq = 1, n_frequencies_bin_edges-1
                        ...
                        ! Calc Eq. 9 of manuscript (em_deriv.pdf)
                        do i_str = 1, n_layers
                            r(i_str) = sum(source(i_str,:,i_freq)*weights_gauss)
                        end do
