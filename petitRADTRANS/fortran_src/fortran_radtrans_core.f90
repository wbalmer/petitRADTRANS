...
double precision, intent(out)   :: contr_em(struc_len,freq_len_p_1-1)
  ...
  INTEGER :: i_mu, i_str, i_freq, tri_n_call
...
!print*,'r',i_iter_scat,r(10),photon_destruct(l,i,10),plancks(10, i),J_star_ini(l, 10, i),source(10)
!if(i_iter_scat == 5) return
              ! Loop over directions
              do j = 1, N_mu
                  ! r(struc_len) = I_J(struc_len) = 0.5[I_plus + I_minus]
                  ! where I_plus is the light that goes downwlards and
                  ...
                  call solve_tridiagonal_system(a(:,j), &
                                  b(:,j), &
                                  c(:,j), &
                                  r, &
                                  struc_len,&
...
!print*,'src',source(10),J_star_ini(l,i,10),J_planet_scat(l,i,10),lambda_loc(l,i,10)
          ...
          do j = 1, N_mu
              flux(i) = flux(i) - I_H(1,j)*mu(j) &
                   * 4d0*cst_pi * w_gauss_ck(l) * w_gauss_mu(j)
          end do
