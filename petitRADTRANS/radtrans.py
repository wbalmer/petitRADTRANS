...
class Radtrans:
    ...
    def __init__(
        ...
        Args:
            ...
            use_detailed_line_absorber_names (Optional[bool]):
                ...
                calculate_spectral_radiosities() and calculate_transit_radii() must match the line absorber names
                exactly, including line list and resolution flags.
        ...
        self._lbl_opacity_sampling = lbl_opacity_sampling
        ...
        self._use_precise_correlated_k_opacities = use_precise_correlated_k_opacities

        self._anisotropic_cloud_scattering = anisotropic_cloud_scattering

        self._use_detailed_line_absorber_names = use_detailed_line_absorber_names
        ...
        self._relative_cloud_scaling_factor = None  # only used as a diagnostic

        # Initialize loaded line opacities variables
        self._line_loaded_opacities = LockedDict()
        self._line_loaded_opacities.update(
            {
                'has_custom_tp_grid': {},
        ...
        self._cia_loaded_opacities = self._init_cia_loaded_opacities(
    ...
    def __check_wavelengths_boundaries(boundaries):
        if np.size(boundaries) != 2:
            raise ValueError(f"wavelengths boundaries must be a list of 2 floats, but was {boundaries}")
    ...
    def __get_cia_contributors(gas_continuum_contributors):
        ...
        for gas_continuum_contributor in gas_continuum_contributors:
            if gas_continuum_contributor in Radtrans._get_non_cia_gas_continuum_contributions():
                continue
    ...
    def line_opacities(self):
        return self._line_loaded_opacities
    ...
    @property
    def use_detailed_line_absorber_names(self):
    ...
    def _compute_non_cia_gas_induced_continuum_opacities(gas_continuum_contributors, mass_fractions,
                                                         pressures, temperatures, frequencies, frequencies_bin_edges,
                                                         mean_molar_masses, **kwargs):
        continuum_opacities = 0

        for gas_continuum_contributor, contribution_function in (
                Radtrans._get_non_cia_gas_continuum_contributions().items()):
            if gas_continuum_contributor in gas_continuum_contributors:
                continuum_opacities += (
                    contribution_function(
                        ...
                        temperatures=temperatures,
                        frequencies=frequencies,
                        frequencies_bin_edges=frequencies_bin_edges,
                        mean_molar_masses=mean_molar_masses,
                        **kwargs
                    )
                )
    ...
    def _init_cia_loaded_opacities(cia_contributors):
        tmp_collision_dict = LockedDict()
        tmp_collision_dict.update({
            'molecules': None,
            'weight': None,
            'lambda': None,
            'temperature': None,
    ...
    def _calculate_cloud_opacities(self, temperatures, cloud_species_mass_fractions, mean_molar_masses, surface_gravity,
        ...
        if "hansen" in cloud_particles_radius_distribution.lower():
            if isinstance(cloud_b_hansen, np.ndarray):
                if not cloud_b_hansen.shape == (self._pressures.size, len(self._cloud_species)):
                    raise ValueError(
                        "cloud_b_hansen must be a float, a dictionary with arrays for each cloud species, "
                        f"or a numpy array with shape {(self._pressures.shape[0], len(self._cloud_species))}, "
                        f"but was of shape {np.shape(cloud_b_hansen)}"
                    )
            elif isinstance(cloud_b_hansen, dict):
                cloud_b_hansen = np.array(list(cloud_b_hansen.values()), dtype='d', order='F').T
            elif isinstance(cloud_b_hansen, float):
                cloud_b_hansen = np.array(
                    np.tile(cloud_b_hansen * np.ones_like(self._pressures), (len(self._cloud_species), 1)),
                    dtype='d',
                    order='F'
                ).T
            else:
        ...
        if clouds_particles_mean_radius is not None or cloud_a_hansen is not None:
            ...
            # Initialize f_seds
            f_seds = np.zeros(len(self._cloud_species))

            for i_spec, cloud in enumerate(self._cloud_species):
        ...
        if self._anisotropic_cloud_scattering:
            continuum_opacities_scattering = cloud_abs_plus_scat_anisotropic - cloud_abs
        else:
            continuum_opacities_scattering = cloud_abs_plus_scat_no_anisotropic - cloud_abs

        if self._scattering_in_emission and hack_cloud_photospheric_optical_depths is not None:
        ...
        if return_cloud_contribution:
            ...
            cloud_opacities = cloud_abs_plus_scat_anisotropic.reshape(opacity_shape)
        else:
            cloud_opacities = None

        return (continuum_opacities, continuum_opacities_scattering,
                hack_cloud_total_scattering_anisotropic, hack_cloud_total_abs, cloud_opacities)
    ...
    def calculate_photosphere_radius(self, planet_radius, temperatures, mean_molar_masses, surface_gravity,
        ...
        try:
            radius_hydrostatic_equilibrium = self.compute_radius_hydrostatic_equilibrium(
            ...
            if self._line_opacity_mode == 'lbl' or self._use_precise_correlated_k_opacities:
                ...
                weights_gauss_reshape = self._line_loaded_opacities['weights_gauss'].reshape(
                    len(self._line_loaded_opacities['weights_gauss']), 1
                )
            ...
            photosphere_radius = -np.ones(self._frequencies.size)
    ...
    def calculate_flux(self, temperatures, mass_fractions, mean_molar_masses, surface_gravity,
                       ...
                       emission_geometry='dayside_ave', stellar_intensity=None,
        ...
        (emission spectrum).
            ...
            Args:
                ...
                cloud_particles_radius_distribution (Optional[string]):
                    ...
                    If hansen, the cloud_b_hansen parameters must be used.
                cloud_a_hansen (Optional[dict]):
                    ...
                    If cloud_a_hansen is not included and dist is "hansen", then it will be computed using Kzz and fsed
                    (recommended).
                cloud_b_hansen (Optional[dict]):
                    ...
                    the particle area (1/cloud_a_hansen^2)
                cloud_f_sed (Optional[float]):
                    cloud settling parameter
                eddy_diffusion_coefficient (Optional[float]):
        ...
        flux, emission_contribution, opacities_rosseland = self._calculate_flux(
            temperatures=temperatures,
            surface_gravity=surface_gravity,
            opacities=opacities,
            continuum_opacities_scattering=continuum_opacities_scattering,
        ...
        if self.__clouds_have_effect(mass_fractions) and return_cloud_contribution:
            cloud_contribution = self._calculate_cloud_optical_depths(
                surface_gravity=surface_gravity,
                pressures=self._pressures,
        ...
        if block2:
            ...
            total_tau_cloud[:, :, :1, :], photon_destruction_prob_cloud = \
                fs.calc_tau_g_tot_ck_scat(
                    ...
                    scattering_in_emission,
                    hack_cloud_total_scattering_anisotropic
                )
    ...
    @staticmethod
    def _compute_optical_depths(pressures, surface_gravity, opacities, continuum_opacities_scattering,
                                line_opacity_mode, scattering_in_emission, use_precise_correlated_k_opacities,
        ...
        if line_opacity_mode == 'lbl' or use_precise_correlated_k_opacities:
            if hack_cloud_photospheric_optical_depths is not None:
                optical_depths, photon_destruction_probabilities, relative_cloud_scaling_factor = (
                    Radtrans._compute_optical_depths_with_custom_cloud(
                        pressures=pressures,
                        surface_gravity=surface_gravity,
                        opacities=opacities,
                        continuum_opacities_scattering=continuum_opacities_scattering,
                ...
                optical_depths[:, :, :1, :], photon_destruction_probabilities = \
                    fs.calc_tau_g_tot_ck_scat(
                        ...
                        opacities[:, :, :1, :],
                        scattering_in_emission,
                        _continuum_opacities_scattering
                    )
    ...
    @staticmethod
    def __compute_pressure_hydrostatic_equilibrium(
            mean_molar_masses, surface_gravity, planet_radius, p0, temperature, radii, rk4=True):
        # TODO is it used?
        p0 = p0 * 1e6
        vs = 1. / radii
    ...
    @staticmethod
    def compute_radius_hydrostatic_equilibrium(pressures, temperatures, mean_molar_masses, surface_gravity,
    ...
    def calculate_rosseland_planck_opacities(self, temperatures, mass_fractions, surface_gravity, mean_molar_masses,
                                             cloud_particle_radius_std=None,
                                             fsed=None, kzz=None, cloud_particles_mean_radii=None, gray_opacity=None,
                                             opaque_layers_top_pressure=None,
                                             power_law_opacity_350nm=None, power_law_opacity_coefficient=None,
                                             haze_factor=1.0, add_cloud_scat_as_abs=False,
                                             dist="lognormal", cloud_b_hansen=None, cloud_a_hansen=None):
        """ Method to calculate the atmosphere's Rosseland and Planck mean opacities.
            ...
            Args:
                ...
                cloud_a_hansen (Optional[dict]):
                    ...
                    If cloud_a_hansen is not included and dist is "hansen", then it will
                ...
                cloud_b_hansen (Optional[dict]):
                    ...
                    distribution normalized by the particle area (1/cloud_a_hansen^2)
        """
        ...
        opacities, continuum_opacities_scattering, _, _, _ = (
            self._calculate_opacities(
                temperatures=temperatures,
                mass_fractions=mass_fractions,
                mean_molar_masses=mean_molar_masses,
        ...
        opacities_rosseland = \
            fs.calc_kappa_rosseland(opacities[:, :, :1, :], temperatures,
                                    self._line_loaded_opacities['weights_gauss'], self._frequencies_bin_edges,
        ...
        return opacities_rosseland, opacities_planck

    def _calculate_flux(self, temperatures, surface_gravity, opacities, continuum_opacities_scattering,
        ...
        """Calculate the flux.
        """
        optical_depths, photon_destruction_probabilities, self._relative_cloud_scaling_factor = (
        ...
        if self._scattering_in_emission:
            ...
            flux, emission_contribution = fs.feautrier_rad_trans(
                ...
                self._line_loaded_opacities['weights_gauss'],
        ...
        return flux, emission_contribution, opacities_rosseland

    @staticmethod
    def _calculate_cloud_optical_depths(surface_gravity, pressures, cloud_opacities):
    ...
    def calculate_transit_radii(self, temperatures, mass_fractions, mean_molar_masses, surface_gravity,
        ...
        opacities, continuum_opacities_scattering, _, _, _ = self._calculate_opacities(
            temperatures=temperatures,
            mass_fractions=mass_fractions,
            mean_molar_masses=mean_molar_masses,
            surface_gravity=surface_gravity,
    ...
    def _compute_cloud_opacities(
        ...
        integrand_absorption = integrand_scale[:, None] * cloud_specs_abs_opa[:, :, None]
        integrand_scattering = integrand_scale[:, None] * cloud_specs_scat_opa[:, :, None]
        integrand_anisotropy = integrand_scattering * (1.0 - cloud_anisotropy[:, :, None])

        widths = np.diff(cloud_rad_bins)[:, None, None, None]  # (P, 1, 1, 1)

        cloud_abs_opa = np.sum(integrand_absorption * widths, axis=(0, 3))  # (Q, M)
        cloud_scat_opa = np.sum(integrand_scattering * widths, axis=(0, 3))  # (Q, M)
        cloud_red_fac_anisotropy = np.sum(integrand_anisotropy * widths, axis=(0, 3))  # (Q, M)

        cloud_red_fac_anisotropy = np.true_divide(
            cloud_red_fac_anisotropy,
            cloud_scat_opa,
        ...
        return cloud_abs_opa, cloud_scat_opa, cloud_red_fac_anisotropy
    ...
    def load_all_opacities(self, start_index=None):
        ...
        if len(self._gas_continuum_contributors) > 0:
            self.load_cia_opacities(
                self._path_input_data,
                self._gas_continuum_contributors
            )

    def load_cia_opacities(self, path_input_data, gas_continuum_contributors):
        for collision in self._cia_loaded_opacities:
            if collision in Radtrans._get_non_cia_gas_continuum_contributions():
                continue
            ...
            if os.path.isfile(hdf5_file):
                with h5py.File(hdf5_file, 'r') as f:
                    ...
                    self._cia_loaded_opacities[collision]['molecules'] = species
                    self._cia_loaded_opacities[collision]['weight'] = np.prod(f['mol_mass'][:])
                    self._cia_loaded_opacities[collision]['lambda'] = wavelengths
                    self._cia_loaded_opacities[collision]['temperature'] = f['t'][:]
                    self._cia_loaded_opacities[collision]['alpha'] = np.transpose(
                        f['cross_sections'][:]
                ...
                # Update keys one-by-one to keep the LockedDict
                self._cia_loaded_opacities[collision]['molecules'] = colliding_species
                self._cia_loaded_opacities[collision]['weight'] = weight
                self._cia_loaded_opacities[collision]['lambda'] = cia_wavelength_grid
                self._cia_loaded_opacities[collision]['temperature'] = cia_temperature_grid
                self._cia_loaded_opacities[collision]['alpha'] = cia_alpha_grid

        print('Done.\n')

    @staticmethod
    def load_hdf5_ktables(file_path_hdf5, frequencies, g_size, temperature_profile_grid_size):
        ...
        with h5py.File(file_path_hdf5, 'r') as f:
            ...
            # have entries.
            ret_val = np.zeros(
                g_size * frequencies.size * temperature_profile_grid_size
            ).reshape(
    ...
    def load_dat_line_opacities(has_custom_line_opacities_temperature_profile_grid, opacities_temperature_profile_grid,
                                size_temperature_profile_grid, custom_line_paths, line_opacity_mode, path_input_data,
        ...
        if not has_custom_line_opacities_temperature_profile_grid:
            ...
            len_tp = size_temperature_profile_grid
        ...
        if line_opacity_mode == 'c-k':
            local_freq_len, local_g_len = fi.load_frequencies_g_sizes(path_input_data, species)
    ...
    def _calculate_opacities(self, temperatures, mass_fractions, mean_molar_masses, surface_gravity,
                             opaque_cloud_top_pressure=None,
        ...
        # Initialization
        self.__scattering_in_transmission = False
        cloud_photosphere_absorption_opacities = None
        cloud_photosphere_scattering_opacities = None
        cloud_opacities = None
        ...
        continuum_opacities += (
            self._compute_cia_opacities(
                cia_dicts=self._cia_loaded_opacities,
                mass_fractions=mass_fractions,
                pressures=self._pressures,
                temperatures=temperatures,
                frequencies=self._frequencies,
        ...
        if self.__clouds_have_effect(mass_fractions):  # add cloud opacity only if there are actually clouds
            ...
            (cloud_continuum_opacities, cloud_continuum_opacities_scattering,
             cloud_photosphere_scattering_opacities, cloud_photosphere_absorption_opacities, cloud_opacities) = (
                self._calculate_cloud_opacities(
                    temperatures=temperatures,
            ...
            if additional_absorption_opacities_function is None:
                if cloud_photosphere_median_optical_depth is not None:
                    cloud_photosphere_absorption_opacities = np.zeros(
                ...
                if cloud_photosphere_median_optical_depth is not None:
                    ...
                    cloud_photosphere_absorption_opacities = cloud_abs

            # Add optional scattering opacity from outside
            if additional_scattering_opacities_function is None:
                if cloud_photosphere_median_optical_depth is not None:
                    cloud_photosphere_scattering_opacities = np.zeros(
                        (self._frequencies.shape[0], self._pressures.shape[0])
                    )
            else:
                ...
                if cloud_photosphere_median_optical_depth is not None:
                    ...
                    # give_absorption_opacity and give_scattering_opacity
                    cloud_photosphere_scattering_opacities = cloud_scattering_opacities
        ...
        opacities = self._interpolate_species_opacities(
            ...
            has_custom_line_opacities_tp_grid=self._line_loaded_opacities['has_custom_tp_grid'],
            line_opacities_temperature_grid_size=self._line_loaded_opacities['temperature_grid_size'],
            line_opacities_pressure_grid_size=self._line_loaded_opacities['pressure_grid_size']
        )
        ...
        for i_spec in range(len(self._line_species)):
            ...
            if self._line_species[i_spec] in mass_fractions or self._use_detailed_line_absorber_names:
        ...
        if self._line_opacity_mode == 'c-k' and self._use_precise_correlated_k_opacities:
            opacities[:, :, 0, :] = fs.combine_opas_ck(
                opacities,
                self._line_loaded_opacities['g_gauss'],
                self._line_loaded_opacities['weights_gauss']
            )
    ...
    def plot_opacities(self,
        ...
        def __compute_opacities(t):
            ...
            weights_gauss = self._line_loaded_opacities['weights_gauss'].reshape(
                (len(self._line_loaded_opacities['weights_gauss']), 1, 1)
            )
        ...
        self._pressures = pressure_bar * 1e6

        wavelengths, opacities = __compute_opacities(temp)
    ...
    def compute_transit_radii(opacities, continuum_opacities_scattering, pressures, temperatures, weights_gauss,
        ...
        radius_hydrostatic_equilibrium = Radtrans.compute_radius_hydrostatic_equilibrium(
            pressures=pressures,
            temperatures=temperatures,
            mean_molar_masses=mean_molar_masses,
            surface_gravity=surface_gravity,
    ...
    def load_line_opacities(self, start_index, path_input_data,
        ...
        if len(self._line_species) > 0:
            ...
            for species in self._line_species:
                ...
                self._line_loaded_opacities['temperature_profile_grid'][species], \
                    custom_line_paths[species], \
                    self._line_loaded_opacities['temperature_grid_size'][species], \
                    self._line_loaded_opacities['pressure_grid_size'][species], \
                    self._line_loaded_opacities['has_custom_tp_grid'][species] \
                ...
                # Read the opacities
                if file_path_hdf5 is None:
                    self._line_loaded_opacities['opacity_grid'][species] = self.load_dat_line_opacities(
                        has_custom_line_opacities_temperature_profile_grid=self._line_loaded_opacities[
                            'has_custom_tp_grid'
                        ][species],
                        opacities_temperature_profile_grid=opacities_temperature_profile_grid,
                        size_temperature_profile_grid=self._line_loaded_opacities['temperature_profile_grid'][
                            species
                        ].shape[0],
                    ...
                    if self._line_opacity_mode == 'c-k':
                        ...
                        self._line_loaded_opacities['opacity_grid'][species] = self.load_hdf5_line_opacity_table(
                            file_path_hdf5=file_path_hdf5,
                            frequencies=self._frequencies,
                            lbl_opacity_sampling=self._lbl_opacity_sampling
                        )
                ...
                self._line_loaded_opacities['opacity_grid'][species] = \
    ...
    def load_cloud_opacities(self, path_input_data):
        ...
        if use_hdf5_files:
            ...
            for i, hdf5_file in enumerate(hdf5_files):
                ...
                with h5py.File(hdf5_file, 'r') as f:
                    if i == 0:
                        ...
                        cloud_anisotropy = np.zeros((cloud_radii.size, cloud_lambdas.size, len(hdf5_files)))
                    ...
                    cloud_anisotropy[:, :, i] = f['asymmetry_parameters'][:]
            ...
            cloud_anisotropy = cloud_anisotropy[:, ::-1, :]
        else:
    ...
    def write_out_rebin(self, resolution, path='', species=None, masses=None):
        ...
        n_spectral_points = int(
            resolution * np.log(self._wavelengths_boundaries[1] / self._wavelengths_boundaries[0]) + 1
        )
        wavenumber_grid = np.logspace(np.log10(1 / self._wavelengths_boundaries[1] / 1e-4),
                                      np.log10(1. / self._wavelengths_boundaries[0] / 1e-4),
                                      n_spectral_points)
        ...
        for spec in species:
            ...
            ret_opa_table = copy.copy(self._line_loaded_opacities['opacity_grid'][spec])
            ...
            ret_opa_table = ret_opa_table.reshape((
                self._line_loaded_opacities['temperature_grid_size'][spec],
                self._line_loaded_opacities['pressure_grid_size'][spec],
                self._frequencies.size,
                len(self._line_loaded_opacities['weights_gauss'])
            ))
            ...
            f.create_dataset('ngauss', data=len(self._line_loaded_opacities['weights_gauss']))
            f.create_dataset('p', data=self._line_loaded_opacities['temperature_profile_grid'][spec][
                                       :self._line_loaded_opacities['pressure_grid_size'][spec], 1] / 1e6)
            f['p'].attrs.create('units', 'bar')
            f.create_dataset('samples', data=self._line_loaded_opacities['g_gauss'])
            f.create_dataset('t', data=self._line_loaded_opacities['temperature_profile_grid'][spec][
                                       ::self._line_loaded_opacities['pressure_grid_size'][spec], 0])
            f.create_dataset('weights', data=self._line_loaded_opacities['weights_gauss'])
